# this script is intended to transform the data, using gaitalytics packages
#it will additionally calcuate the COM and add this channel to the .nc file after
#!/usr/bin/env python3
...
class TrialProcess:
    ...
    @staticmethod
    def _clean_events(raw_events, min_gap=0.2):
        """
        Remove back-to-back duplicate strikes/offs and events closer than min_gap s.
        """
        import pandas as pd

        cleaned = []
        for side, grp in raw_events.groupby('context'):
            ...
            for _, row in grp.iterrows():
                if row.label == last_label:
                    continue
                if last_time is not None and (row.time - last_time) < min_gap:
                    continue
                cleaned.append(row)
        ...
        return df.sort_values(['time','context']).reset_index(drop=True)

    def get_events(self, clean=True, min_gap=0.2):
        """
        Detect and (optionally) clean gait events.
        Always returns a DataFrame (raw or cleaned), never raises ValueError again.
        """
        # Ensure the trial+COM is computed
        if self.trial is None:
            self.compute()

        # 1) Detect raw events
        raw = api.detect_events(self.trial, self.config)

        # By default we’ll use raw unless cleaning succeeds
        events = raw

        # 2) First check on raw
        try:
            api.check_events(raw)
            print(f"✔️ Raw events valid for {self.c3d_path.name}")

        except ValueError as e1:
            print(f"⚠️ Initial event check failed for {self.c3d_path.name}: {e1}")

            if clean:
                # 3) Clean the table
                cleaned = self._clean_events(raw, min_gap=min_gap)

                # 4) Try check again on cleaned
                try:
                    api.check_events(cleaned)
                    events = cleaned
                    print(f"✔️ Cleaned events valid for {self.c3d_path.name}")

                except ValueError as e2:
                    # If still invalid, warn but proceed with cleaned
                    print(f"❌ Cleaned events still invalid: {e2}")
                    events = cleaned

            else:
                # If user disabled cleaning, just proceed with raw (or raise)
                print("Proceeding with raw events despite errors")
                # events stays as raw

        # 5) Attach and return whichever table we ended up with
        self.events = events
        self.trial.events = events
        return events
