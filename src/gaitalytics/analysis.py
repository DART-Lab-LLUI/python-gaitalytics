...
class _CycleDurationAnalysis(AbstractAnalysis):
    ...
    def _calculate_cycle_duration(self, meta_data: dict[str, any], context: model.GaitEventContext) -> dict[
        ...
        double_support_duration = (meta_data["Foot Off_CONTRA"] + (
                meta_data["Foot Off_IPSI"] - meta_data["Foot Strike_CONTRA"])) / frequency
        single_support_duration = ((meta_data["Foot Strike_CONTRA"] - meta_data["Foot Off_CONTRA"]) + (
                cycle_length - meta_data["Foot Off_IPSI"])) / frequency
